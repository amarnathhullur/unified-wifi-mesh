...
dm_easy_mesh_t *dm_easy_mesh_list_t::create_data_model(const char *net_id, const em_interface_t *al_intf, em_profile_type_t profile, bool controller)
{
    ...
    if (controller == true) {
        ...
        if (dm_easy_mesh_t::name_from_mac_address(&al_intf->mac, name) == 0) {
            ...
            if ((net = get_network(net_id)) != NULL) {
                ref_dm = get_data_model(GLOBAL_NET_ID, net->m_net_info.ctrl_id.mac);
                assert(ref_dm != NULL);
                pnet = ref_dm->get_network();
                *pnet = *net;
                //if colocated, set the agent interface mac address in controller's dm
                ref_dm->m_network.set_colocated_agent_interface_mac((unsigned char*)al_intf->mac);
                em_printfout("Set colocated agent interface mac %s in controller dm", util::mac_to_string(ref_dm->m_network.get_colocated_agent_interface_mac()).c_str());
                //also trigger db update of networklist for colocated agent id
                ref_dm->set_db_cfg_param(db_cfg_type_network_list_update, "");
            }
        }
    }
	...
	if (controller == true) {
		...
		dev->update_easymesh_json_cfg(controller);
	}
    ...
    if ((net = get_network(net_id)) != NULL) {
        ...
        *pnet = *net;
        ...
        if(ref_dm != NULL) {
            ...
            em_printfout("Reference data model not found for network: %s and ctrl AL mac: %s", net->m_net_info.id, util::mac_to_string(net->m_net_info.ctrl_id.mac).c_str());
        }
    }
    printf("%s:%d: Putting data model at key: %s\n", __func__, __LINE__, key);
